...
// ─────────────────────────────────────────────────────────────
// HELPERS  (local UI wrappers — no more mutable fields)
// ─────────────────────────────────────────────────────────────
...
class _LinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxAmt =
        entries.map((e) => e.amount).reduce((a, b) => a > b ? a : b);
    if (maxAmt == 0) return;
    ...
    for (final cat in categories) {
      final catEntries =
          entries.where((e) => e.categoryName == cat.name).toList();
      if (catEntries.length < 2) continue;

      final pts = catEntries.asMap().entries.map((me) {
        final i = me.key;
        final e = me.value;
        final x = padding.left + chartW * i / (catEntries.length - 1);
        final y = padding.top + chartH * (1 - e.amount / maxAmt);
        return Offset(x, y);
      }).toList();
    ...
    for (int i = 0; i < entries.length; i++) {
      final x =
          padding.left + chartW * i / math.max(entries.length - 1, 1);
      final y = size.height - 14;
      final parts = _formatDate(entries[i].date).split(' ');
